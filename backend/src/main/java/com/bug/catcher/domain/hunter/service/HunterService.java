...
public class HunterService {//등급 산정 엔진과 외부 공개 프로필 조회용
    // 등급/완료 수 집계는 "완료" 상태 의뢰만 포함한다.
    private static final String COMPLETED_STATUS = "완료";
    ...
    public void updateHunterLevel(Long hunterId) {
        ...
        long completedCount = applicationRepository.countByHunterIdAndRequest_Status(hunterId, COMPLETED_STATUS);
        float averageRating = reviewRepository.getAverageRatingByHunterId(hunterId);

        String newGrade = calculateGrade(completedCount, averageRating);

        // 산정된 등급으로 즉시 업데이트
        hunter.updateGrade(newGrade);
        hunter.syncCompletionCount(completedCount);
    }
    ...
    public HunterProfileResponseDto getHunterProfile(Long hunterId) {
        ...
        return new HunterProfileResponseDto(hunter, completedCount, averageRating);
    }
    ...
    public Page<HunterListResponseDto> getHunterList(Long userId, Pageable pageable) {
        ...
        return hunters.map(hunter -> new HunterListResponseDto(
                ...
                applicationRepository.countByHunterIdAndRequest_Status(hunter.getId(), COMPLETED_STATUS)
        ));
    }
    //헌터가 받은 리뷰 조회
    @Transactional(readOnly = true)
